...
public class Reviser {
	...
	public Reviser(int t){
		...
		type = connection.getColumnAsArray("GA_TRIPS", "V_TYP").clone();
	}
	...
	public boolean checkTypeConstraint(int trip, int depot){
		if(type[trip]==depot) return true;
		else return false;
	}
	
	public int checkConstraints(int[] I, int start_index){ //I sollte ein trips*2 Array sein
		for(int i=start_index;i<I.length;i=i+2){
			//p(i);
			int current_depot 		= I[i];
			int current_vehicle 	= I[i+1];
			int current_start_time 	= start_time[i/2];
			int current_type 		= type[i/2];
